...
const ListContent2 = ({ title, arrOfCards, itemsPerPage }) => {
  const [position, setPosition] = useState(0)
  const [windowWidth, setWindowWidth] = useState(window.innerWidth)
  let swipe = false;
  let items = itemsPerPage
  if (windowWidth <= 1992) items = 5;
  if (windowWidth <= 1712) items = 4;
  if (windowWidth <= 1294) items = 3;
  if (windowWidth <= 958) items = 2;
  const detectWidth = () => {
